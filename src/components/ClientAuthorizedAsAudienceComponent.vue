...
<template>
  <div>
    ...
    <ClientAuthorizedRemoveScopeModal
      ref="authorizedRemoveScopeModal"
      v-if="client"
      :client="client"
      @refreshClient="refreshClient"
    />
    <ClientAuthorizedAddScopeModal
      ref="authorizedAddScopeModal"
      v-if="client"
      :client="client"
      @refreshClient="refreshClient"
    />
    ...
    <table class="table table-striped table-hover mt-2">
      ...
      <thead class="table-dark">
        <tr>
          ...
          <th scope="col" class="text-end">
            <button
              v-if="client.managementPermissions.canAddAuthorization"
              type="button"
              class="btn btn-success float-end"
              @click="resetAuthorize()"
              data-bs-toggle="modal"
              data-bs-target="#clientAuthorizeModal"
            >
      ...
      <tbody>
        ...
        <tr v-for="authorization in client.authorizationsAsAudience" :key="authorization.id">
          <td>
            <p class="text-break">{{ authorization.subject.clientId }}</p>
          </td>
          <td>
            <div class="d-flex align-items-center flex-wrap my-button-group">
              <div
                ...
                role="group"
                v-for="scope in authorization.authorizedScopes"
                :key="scope"
              >
                <span class="btn btn-dark btn-sm custom-disabled">{{ scope }}</span>
                <button
                  v-if="client.managementPermissions.canAuthorizeRemoveScope"
                  type="button"
                  class="btn btn-danger btn-sm"
                  @click="removeScope(authorization.subject.clientId, scope)"
                  data-bs-toggle="modal"
                  data-bs-target="#authorizedRemoveScopeModal"
                >
              ...
              <button
                v-if="
                  client.managementPermissions.canAuthorizeAddScope &&
                  authorization.authorizedScopes.length < client.availableScopes.length
                "
                type="button"
                class="btn btn-success btn-sm me-2 mb-2"
                @click="addScope(authorization.subject.clientId, authorization.authorizedScopes)"
                data-bs-toggle="modal"
                data-bs-target="#authorizedAddScopeModal"
              >
                <i class="bi bi-plus"></i>
              </button>
            </div>
          ...
          <td class="text-end">
            <div class="btn-group" role="group">
              <button
                v-if="client.managementPermissions.canDeleteAuthorization"
                type="button"
                class="btn btn-danger float-end"
                @click="openDeauthorizeModal(authorization.subject.clientId)"
                data-bs-toggle="modal"
                data-bs-target="#clientDeauthorizeModal"
              >
                <i class="bi bi-trash"></i>
              </button>
              <router-link
                :to="'/clients/' + encodeURIComponent(authorization.subject.clientId)"
